...
로컬에서는 Celery·Redis로 에이전트를 실행하고, AWS 배포 시에는 FastAPI와 DB 계약을 유지한 채 실행 adapter만 AgentCore로 교체하는 구조다.
...
| `joungs` | main 기능 전체 + 서비스 DB 모델 + 루트 FastAPI API + 인증 + 대시보드/프론트 연동 + Celery 실행 기반 |

현재 `joungs`에는 최신 `main`이 병합되어 있다. 따라서 앞으로는 에이전트 코드를 다시 옮기는 작업보다, **main의 agent runtime을 joungs의 pipeline DB·Celery 흐름에 연결하는 작업**이 핵심이다.
...
flowchart LR
    ...
    Runner --> Local["LocalCeleryRunner"]
    Local --> Redis["Redis"]
    Redis --> Worker["Celery Worker"]

    Runner -. AWS 전환 .-> Core["AgentCoreRunner"]
    Core --> AgentCore["AWS AgentCore"]

    Worker --> Supervisor["Orchestrator / Supervisor"]
...
`AgentRunner`는 실행 환경을 감추는 공통 계약이다.

- 로컬: `FastAPI → Redis → Celery Worker → Agent Runtime`
- AWS: `FastAPI → AgentCore → Agent Runtime`
- 공통: 실행 event, output, metric을 같은 형식으로 PostgreSQL에 기록
...
DB 모델은 준비됐지만, 요청 생성 후 Celery task를 enqueue하고 상태를 갱신하는 연결은 아직 없다.
...
│   └── pipeline/              # Celery app, task, AgentRunner 계약
├── agent_runtime/
...
다음 단계는 두 코드를 합쳐 새 구조를 만드는 것이 아니라, Supervisor를 `AgentRunner` 뒤에 adapter로 연결하는 것이다.
...
- 대시보드와 프론트 연동
- Redis·Celery worker 실행 환경

### 다음 통합 순서

1. `POST /api/v1/data-requests` 이후 `execute_stage` Celery task enqueue
...
7. AWS 배포 시 Celery adapter를 AgentCore adapter로 교체

## 8. 팀 개발 기준

- agent runtime은 FastAPI ORM을 직접 import하지 않는다.
- 실행 상태의 기준은 `pipeline_runs → stage_runs → pipeline_events`다.
- 프론트는 Celery task ID나 AgentCore session ID를 직접 알지 않는다.
- 외부 실행 ID는 `stage_runs.executor_reference`에만 저장한다.
