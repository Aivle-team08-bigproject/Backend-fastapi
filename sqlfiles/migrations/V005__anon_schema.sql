-- =====================================================================
-- [V005] anonymized 스키마 — 익명 데이터 계층
-- 대상: PostgreSQL 17, portfolio DB 안에 mart와 나란히 존재
--
-- 구성: 테이블 + CHECK 22종 + mcc 인덱스 + 미래날짜 CHECK 3종 +
--   mart(V004)와 동일한 정합성 트리거 5종.
--   "mart에 있는 안전망은 anonymized에도 전부 있다" 원칙 (2026-07 확정).
--
...
--   ※ 반드시 portfolio_admin으로 실행 — 소유자만 이후 TRUNCATE·트리거 제어가 가능하며,
...
-- anonymized 테이블은 대응하는 mart 테이블과 row 수·PK가 항상 1:1이어야 한다
-- (LLM이 "없는 데이터"를 찾아 헤매지 않도록).
--
-- FK는 전부 anonymized 스키마 내부끼리만 참조한다(= mart를 참조하지 않음).
-- 이렇게 하는 이유: agent_svc에게 anonymized 스키마만 GRANT해도 완결적으로 동작해야
-- 하기 때문 — mart를 참조하면 agent_svc가 mart 접근권한도 필요해져
-- "LLM은 mart에 직접 접근하지 않는다"는 원칙이 깨진다.
--
-- ---------------------------------------------------------------------
-- 스키마 이름이 왜 anon 이 아니라 anonymized 인가 (2026-07-29 변경)
-- ---------------------------------------------------------------------
-- `anon`은 PostgreSQL Anonymizer 확장의 표준 스키마 이름이라, 일부 관리형
-- PostgreSQL이 예약어처럼 보호한다. Neon에서 아래 에러로 실제로 막혔다.
--
--     ERROR: cannot create trigger on relation "transactions":
--            triggers on the anon schema are not permitted
--
-- 파일명(V005__anon_schema.sql)과 배치 경로(scripts/anon_batch/)는 그대로 둔다 —
-- 이미 적용한 환경과 기록이 어긋나지 않게 하기 위함이며, 스키마 이름이 아니다.
-- 기존 DB는 V012가 ALTER SCHEMA 한 줄로 이름만 바꾼다(데이터·코멘트·권한 보존).
-- =====================================================================

BEGIN;

CREATE SCHEMA IF NOT EXISTS anonymized;
...
목표 k=3 (2026-07 기준, 검증 데이터 800명 규모 대비 선정). 데이터 규모가 2만 명으로
커지면 재검토 가능하나, 규모가 커질수록 동일 k를 유지하는 데 드는 손실은 오히려
줄어드는 구조이므로 k=3을 상향해야만 하는 강제 사유는 아님.

customer_id는 mart의 값을 그대로 쓰지 않고 별도 토큰으로 재생성한다 — 원본을 유지하면
두 계층을 잇는 결합 키가 남아 "다른 정보와 결합해도 개인을 알아볼 수 없어야 한다"는
익명정보 요건이 흔들리기 때문. 고객 단위 집계는 이 토큰으로 그대로 가능하다.
...
-- ---------------------------------------------------------------------
-- 1. anonymized.mcc_codes — mart와 완전 동일 (개인정보 아님, 순수 참조 데이터)
-- ---------------------------------------------------------------------
CREATE TABLE anonymized.mcc_codes (
...
-- ---------------------------------------------------------------------
CREATE TABLE anonymized.customers (
...
mart처럼 고정 7개 구간이 아니라, anonymized에서는 실제 존재하는 값의 종류가 배치 결과에
...
-- ---------------------------------------------------------------------
-- 3. anonymized.cards — mart와 동일 구조 (핵심 준식별자 조합 대상 아님)
-- ---------------------------------------------------------------------
CREATE TABLE anonymized.cards (
    ...
    CONSTRAINT fk_anon_cards_customer FOREIGN KEY (customer_id)
        REFERENCES anonymized.customers (customer_id)
);

COMMENT ON TABLE anonymized.cards IS
'mart.cards와 동일 구조. FK는 anonymized.customers만 참조(mart 미참조).
카드 속성 자체는 현재 핵심 준식별자 조합 대상이 아니므로 mart 값 그대로 유지.';


-- ---------------------------------------------------------------------
-- 4. anonymized.merchants — mart와 동일 구조
-- ---------------------------------------------------------------------
CREATE TABLE anonymized.merchants (
    ...
    CONSTRAINT fk_anon_merchants_mcc FOREIGN KEY (mcc_code)
        REFERENCES anonymized.mcc_codes (mcc_code)
);
...
-- ---------------------------------------------------------------------
CREATE TABLE anonymized.transactions (
    ...
    CONSTRAINT fk_anon_tx_card FOREIGN KEY (card_number_masked)
        REFERENCES anonymized.cards (card_number_masked),
    CONSTRAINT fk_anon_tx_merchant FOREIGN KEY (merchant_id)
        REFERENCES anonymized.merchants (merchant_id),
...
-- 인덱스: mart와 동일한 최소 구성
CREATE INDEX idx_anon_tx_card     ON anonymized.transactions (card_number_masked);
CREATE INDEX idx_anon_tx_merchant ON anonymized.transactions (merchant_id);
CREATE INDEX idx_anon_tx_datetime ON anonymized.transactions (transaction_datetime);
...
-- ---------------------------------------------------------------------
ALTER TABLE anonymized.merchants
...
-- ---------------------------------------------------------------------
ALTER TABLE anonymized.transactions
...
-- ---------------------------------------------------------------------
CREATE INDEX idx_anon_tx_mcc ON anonymized.transactions (mcc_code);
...
-- ---------------------------------------------------------------------
-- 정합성 트리거 5종 — mart V3와 동일 로직의 anonymized 버전
--
-- 필요한 이유: anonymized은 익명화 배치 스크립트가 채우는데, CHECK는 "같은 행 안"
-- 규칙만 잡는다. 배치가 조인을 잘못 걸어 카드-거래 짝이 어긋난 채 들어가는
-- 유형의 버그는 다른 테이블 참조가 필요해 트리거만 잡을 수 있다.
-- ("mart에 있는 안전망은 anonymized에도 전부 있다" 원칙)
--
-- 대량 적재 시: mart와 동일하게 DISABLE TRIGGER → 적재 → 검증 → ENABLE 절차 사용.
-- ---------------------------------------------------------------------

-- [1] 거래일시 >= 카드 발급월
CREATE OR REPLACE FUNCTION anonymized.fn_tx_after_card_issue()
RETURNS trigger AS $$
DECLARE
    v_issue VARCHAR(7);
BEGIN
    SELECT card_issue_month INTO v_issue
    FROM anonymized.cards WHERE card_number_masked = NEW.card_number_masked;

    IF v_issue IS NOT NULL
       AND to_char(NEW.transaction_datetime, 'YYYY-MM') < v_issue THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 카드 발급 전 거래: transaction_id=%, 거래=%, 발급월=%',
...
CREATE TRIGGER trg_anon_tx_after_card_issue
    BEFORE INSERT OR UPDATE ON anonymized.transactions
    FOR EACH ROW EXECUTE FUNCTION anonymized.fn_tx_after_card_issue();

-- [2] 해지된 카드는 해지월 이후 거래 불가
CREATE OR REPLACE FUNCTION anonymized.fn_tx_before_card_terminated()
RETURNS trigger AS $$
DECLARE
    v_status  VARCHAR(10);
    v_changed VARCHAR(7);
BEGIN
    ...
    IF v_status = '해지' AND v_changed IS NOT NULL
       AND to_char(NEW.transaction_datetime, 'YYYY-MM') > v_changed THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 해지 카드의 해지 후 거래: transaction_id=%, 거래=%, 해지월=%',
...
CREATE TRIGGER trg_anon_tx_before_card_terminated
    BEFORE INSERT OR UPDATE ON anonymized.transactions
    FOR EACH ROW EXECUTE FUNCTION anonymized.fn_tx_before_card_terminated();

-- [3] 거래일시가 가맹점 영업 기간 내 (해외거래는 검사 제외)
CREATE OR REPLACE FUNCTION anonymized.fn_tx_merchant_active()
RETURNS trigger AS $$
DECLARE
    v_open    VARCHAR(7);
    v_status  VARCHAR(10);
...
BEGIN
    ...
    FROM anonymized.merchants WHERE merchant_id = NEW.merchant_id;

    IF v_open IS NOT NULL AND v_month < v_open THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 가맹점 개업 전 거래: transaction_id=%, 거래=%, 개업월=%',
    ...
    IF v_status = '폐업' AND v_changed IS NOT NULL AND v_month > v_changed THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 폐업 가맹점의 폐업 후 거래: transaction_id=%, 거래=%, 폐업월=%',
...
CREATE TRIGGER trg_anon_tx_merchant_active
    BEFORE INSERT OR UPDATE ON anonymized.transactions
    FOR EACH ROW EXECUTE FUNCTION anonymized.fn_tx_merchant_active();

-- [4] 국내거래 MCC 일치 (해외거래는 검사 제외)
CREATE OR REPLACE FUNCTION anonymized.fn_tx_mcc_match()
RETURNS trigger AS $$
DECLARE
    v_mcc INTEGER;
BEGIN
    ...
    IF v_mcc IS NOT NULL AND v_mcc <> NEW.mcc_code THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 국내거래 MCC 불일치: transaction_id=%, 거래MCC=%, 가맹점MCC=%',
...
CREATE TRIGGER trg_anon_tx_mcc_match
    BEFORE INSERT OR UPDATE ON anonymized.transactions
    FOR EACH ROW EXECUTE FUNCTION anonymized.fn_tx_mcc_match();

-- [5] 체크카드 할부 금지
CREATE OR REPLACE FUNCTION anonymized.fn_tx_check_card_no_installment()
RETURNS trigger AS $$
DECLARE
...
BEGIN
    ...
    IF v_product LIKE 'CK%' THEN
        RAISE EXCEPTION
            '[anonymized 정합성 위반] 체크카드 할부 거래: transaction_id=%, 상품=%, 할부=%개월',
...
CREATE TRIGGER trg_anon_tx_check_card_no_installment
    BEFORE INSERT OR UPDATE ON anonymized.transactions
    FOR EACH ROW EXECUTE FUNCTION anonymized.fn_tx_check_card_no_installment();

COMMIT;

-- ============================================================
-- 권한 부여 (GRANT) — agent_svc는 anonymized만, 읽기 전용
-- ============================================================
GRANT USAGE ON SCHEMA anonymized TO agent_svc;
GRANT SELECT ON ALL TABLES IN SCHEMA anonymized TO agent_svc;
ALTER DEFAULT PRIVILEGES IN SCHEMA anonymized
    GRANT SELECT ON TABLES TO agent_svc;

REVOKE ALL ON SCHEMA anonymized FROM PUBLIC;
