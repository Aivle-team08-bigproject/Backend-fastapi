...
def _load_runtime_secrets() -> None:
    ...
    if bundle_arn:
        ...
        for key in (
            "DATABASE_URL",
            "HANACARD_APP_DATABASE_URL",
            "HANACARD_AGENT_DATABASE_URL",
            "JWT_SECRET",
            "ANON_HASH_SALT",
            "DEEPSEEK_API_KEY",
        ):
    ...
    db_arn = os.getenv("HANACARD_APP_DATABASE_SECRET_ARN", "").strip()
    if db_arn:
        os.environ["HANACARD_APP_DATABASE_URL"] = _normalize_psycopg_url(_secret_string(db_arn))
...
class Settings(BaseSettings):
    ...
    # hanacard_admin: Alembic 마이그레이션·익명화 배치 전용
