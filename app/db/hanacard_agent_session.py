"""hanacard DB, agent_svc 계정 전용 비동기 SQLAlchemy 세션.

anon 스키마 읽기 + service 스키마 중 파이프라인 로그성 테이블(읽기/쓰기)에 사용한다.
app_svc 세션(app/db/hanacard_app_session.py)과 엔진을 공유하지 않는다 —
...
engine = create_async_engine(
    settings.hanacard_agent_database_url, echo=False, future=True, pool_pre_ping=True
)
