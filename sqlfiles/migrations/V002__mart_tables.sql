...
(GRANT 대상 아님, agent_svc는 anonymized 스키마만 사용).
...
k-익명성: 준식별자(성별+연령대+거주지역) 조합에 1명뿐인 경우가 존재하여 현재 k=1.
  ...
  외부 제공용 익명정보를 생성할 때는 k=3을 확보해야 함(anonymized 계층에서 처리).
  근거: 본 데이터(800명) 실측 결과 k=3은 손실 12.8%, k=5는 손실 38.1%로 과도함.
  금융분야 가명·익명처리 안내서의 카드사 사례는 k=5를 쓰나, 그것은 외부 일반사업자에게
  제공하는 환경이다. 안내서 자체가 "k값은 익명정보 이용 목적·환경 등에 따라 상이"하다고
  명시하고 있으며, 본 데이터는 내부 분석용이고 규모가 작아 k=3을 채택함.
