-- =====================================================================
-- [P001] 기존 DB 보정 — 이미 구축된 hanacard를 새 표준에 맞춘다
--
...
--   되었고, 그 결과 hanacard_admin(마이그레이션·배치 전용 계정)이
...
--   hanacard_admin으로 이전한다. (V14__ownership_transfer.sql을 흡수·확장한 것 —
...
--
-- 실행: psql -v ON_ERROR_STOP=1 -d hanacard -f P001__align_existing_db.sql
...
-- 이 파일을 고칠 필요가 없고, 이미 hanacard_admin 소유인 것은 건너뛴다.
-- ---------------------------------------------------------------------
DO $$
DECLARE
    r record;
    ...
    FOR r IN
        SELECT schemaname, tablename
         ...
         WHERE schemaname IN ('mart', 'anon', 'service')
           AND tableowner <> 'hanacard_admin'
    LOOP
        EXECUTE format('ALTER TABLE %I.%I OWNER TO hanacard_admin',
    ...
    FOR r IN
        SELECT n.nspname AS schema_name,
         ...
         WHERE n.nspname IN ('mart', 'anon', 'service')
           AND pg_get_userbyid(p.proowner) <> 'hanacard_admin'
    LOOP
        EXECUTE format('ALTER FUNCTION %s OWNER TO hanacard_admin', r.func_sig);
        n := n + 1;
    END LOOP;
...
-- ---------------------------------------------------------------------
ALTER SCHEMA mart    OWNER TO hanacard_admin;
ALTER SCHEMA anon    OWNER TO hanacard_admin;
ALTER SCHEMA service OWNER TO hanacard_admin;

-- ---------------------------------------------------------------------
-- 4. V001의 설정을 기존 DB에도 적용 (누락분 보정)
-- ---------------------------------------------------------------------
GRANT CREATE, CONNECT ON DATABASE hanacard TO hanacard_admin;
...
-- =====================================================================
-- 1) 소유자가 hanacard_admin이 아닌 테이블
-- SELECT schemaname, tablename, tableowner FROM pg_tables
--  WHERE schemaname IN ('mart','anon','service') AND tableowner <> 'hanacard_admin';
--
-- 2) 소유자가 hanacard_admin이 아닌 함수
