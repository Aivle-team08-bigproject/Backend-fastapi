#!/usr/bin/env bash
# =====================================================================
# hanacard DB 부트스트랩 — mart/anon(SQL) + service(Alembic)를 순서대로 구축
#
# 전제:
#   1) hanacard 데이터베이스가 이미 생성돼 있을 것 (docker compose가 생성)
#   2) PostgreSQL superuser 접속 정보가 설정돼 있을 것
#   3) 역할 비밀번호가 AGENT_SVC_PASSWORD / APP_SVC_PASSWORD /
#      HANACARD_ADMIN_PASSWORD 환경변수에 설정돼 있을 것
#   4) 레포의 .env에 HANACARD_MIGRATION_DATABASE_URL이 설정돼 있을 것 (Alembic용)
#
# 사용:
...
DB="${PGDATABASE:-hanacard}"
ADMIN_USER="${ADMIN_USER:-hanacard_admin}"
...
password_for() {  # password_for <실행계정>
    case "$1" in
        ...
        hanacard_admin) printf '%s' "${HANACARD_ADMIN_PASSWORD:-}" ;;
...
if [[ "$CREATE_ROLES" -eq 1 ]]; then
    ...
    create_role "hanacard_admin" "HANACARD_ADMIN_PASSWORD"
fi
...
# --- 2. mart / anon (hanacard_admin) ----------------------------------
# 반드시 hanacard_admin으로 실행한다 — 실행 계정이 곧 테이블 소유자가 되고,
