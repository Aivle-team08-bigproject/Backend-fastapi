...
class FakeSecretsManager:
    ...
    def get_secret_value(self, *, SecretId):
        ...
        return {"SecretString": "postgresql+psycopg://agent_svc:password@example.rds.amazonaws.com/portfolio?sslmode=require"}
...
def test_runtime_database_reads_rds_postgres_json(monkeypatch):
    ...
    fake_client.get_secret_value = lambda **kwargs: {
        "SecretString": '{"host":"db.rds.amazonaws.com","port":5432,"dbname":"portfolio","username":"app_svc","password":"p@ss"}'
    }
    captured = {}
    ...
    assert captured["url"] == "postgresql+asyncpg://app_svc:p%40ss@db.rds.amazonaws.com:5432/portfolio?sslmode=require"


def test_runtime_database_reads_single_key_connection_string_json(monkeypatch):
    fake_client = FakeSecretsManager()
    fake_client.get_secret_value = lambda **kwargs: {
        "SecretString": '{"bigproject/local-dev/agentcore/neon-database-url":"postgresql://agent_svc:password@example.neon.tech/portfolio?sslmode=require"}'
    }
    captured = {}
    ...
    asyncio.run(RuntimeDatabase().initialize())
    assert captured["url"].startswith("postgresql+asyncpg://agent_svc:password@example.neon.tech/")
