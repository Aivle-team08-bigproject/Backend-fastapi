...
--   ※ 반드시 hanacard_admin으로 실행할 것 — PostgreSQL은 CREATE TABLE을 실행한
...
— 하나카드가 직접 계약한 가맹점만 존재하므로, 해외 거래는 transactions.merchant_id가
NULL로 기록된다.
...
하나카드가 부여하는 내부 식별자이므로 해외 가맹점에는 존재하지 않는다.
