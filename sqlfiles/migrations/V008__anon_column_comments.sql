-- =====================================================================
-- [V008] anon 스키마 — 테이블/컬럼 COMMENT 전면 정비
...
-- 왜 이 파일이 필요한가
-- ---------------------------------------------------------------------
-- agent_svc는 anon 스키마의 테이블/컬럼 COMMENT를 읽을 수 있고, 그 텍스트가
-- 스키마-온리 프롬프팅을 통해 LLM 컨텍스트에 그대로 들어간다.
-- 즉 anon의 COMMENT는 "DBA용 설계 메모"가 아니라 "LLM에게 주는 분석 가이드"다.
--
-- 작성 원칙
...
COMMENT ON COLUMN anon.mcc_codes.mcc_code IS
'업종 코드. PK. ISO 18245 표준 코드값.';

COMMENT ON COLUMN anon.mcc_codes.mcc_name IS
'업종명. 코드와 1:1 대응.
...
COMMENT ON COLUMN anon.customers.customer_id IS
...
COMMENT ON COLUMN anon.cards.card_status IS
'카드 상태. 값: 정상, 일시정지, 해지.
조회 시점의 현재 상태이며, 변경 시점은 card_status_changed_month를 볼 것.';

COMMENT ON COLUMN anon.cards.card_status_changed_month IS
...
COMMENT ON COLUMN anon.cards.signup_channel IS
...
COMMENT ON COLUMN anon.merchants.merchant_id IS
'가맹점 식별자. PK. 형식 MER_+12자리 HEX.
카드사 내부 식별자에 해당하므로 해외 가맹점에는 존재하지 않는다.
transactions.merchant_id가 참조.';

COMMENT ON COLUMN anon.merchants.merchant_name IS
...
COMMENT ON COLUMN anon.merchants.merchant_region IS
...
COMMENT ON COLUMN anon.merchants.merchant_open_month IS
'가맹 계약월(사업자등록월). YYYY-MM 형식.
이 값보다 이른 시점의 거래는 존재하지 않는다.';

COMMENT ON COLUMN anon.merchants.fee_tier_code IS
'카드 수수료율 구분. 값: 영세, 중소1, 중소2, 일반.
여신전문금융업법상 영세·중소 가맹점 우대수수료 체계.
가맹점 규모의 대리 지표로도 쓸 수 있다(영세일수록 소규모).';

COMMENT ON COLUMN anon.merchants.merchant_status IS
'가맹점 상태. 값: 정상, 휴업, 폐업.
현재 영업 중인 가맹점만 집계하려면 이 컬럼으로 필터할 것.';

COMMENT ON COLUMN anon.merchants.merchant_status_changed_month IS
...
COMMENT ON COLUMN anon.transactions.transaction_id IS
'거래 일련번호. PK. 형식 TX + 8자리 숫자. 거래 건수는 이 컬럼으로 센다.';

COMMENT ON COLUMN anon.transactions.card_number_masked IS
'거래에 사용된 카드. NOT NULL. cards.card_number_masked 참조.
고객 정보는 이 컬럼으로 cards를 조인해 얻는다.';

COMMENT ON COLUMN anon.transactions.merchant_id IS
...
COMMENT ON COLUMN anon.transactions.mcc_code IS
...
COMMENT ON COLUMN anon.transactions.transaction_datetime IS
...
COMMENT ON COLUMN anon.transactions.approval_status IS
'승인 결과. 값: 승인, 거절.
주의 — 매출 집계 시 반드시 승인 건만 필터할 것.
거절 건에도 시도 금액과 할부 개월이 기록돼 있다(승인 요청 시점의 조건이므로 정상).
승인율 = 승인 건수 / 전체 건수로 계산 가능.';

COMMENT ON COLUMN anon.transactions.decline_reason_code IS
...
COMMENT ON COLUMN anon.transactions.transaction_amount IS
...
COMMENT ON COLUMN anon.transactions.currency_code IS
'거래 통화 (ISO 4217). 값: KRW, USD, EUR, JPY, THB, SGD, PHP, VND.
KRW는 전부 국내거래다.';

COMMENT ON COLUMN anon.transactions.krw_converted_amount IS
...
COMMENT ON COLUMN anon.transactions.applied_exchange_rate IS
'거래 시점 적용 환율 (1 외화 = N원). KRW는 1.0.
주의 — 현재 데이터는 통화별 고정값이라 기간 중 환율 변동이 없다. 시계열 환율 분석 불가.
실제로는 거래 시점 스냅샷을 저장하는 것이 정상이므로 중복 컬럼이 아니다.';

COMMENT ON COLUMN anon.transactions.merchant_country_code IS
...
COMMENT ON COLUMN anon.transactions.pos_entry_mode IS
...
COMMENT ON COLUMN anon.transactions.auth_method IS
...
--  WHERE table_schema = 'anon'
--    AND col_description(('anon.' || table_name)::regclass, ordinal_position) IS NULL;
--
