...
-- 이 파일이 하는 일:
--   1) 타임존 UTC 고정
--   2) hanacard_admin에게 스키마 생성 권한 부여 (V002~ 실행에 필요)
--   3) 스키마 3개(mart/anon/service)를 hanacard_admin 소유로 생성
--   4) 앞으로 생성될 테이블의 기본 권한(DEFAULT PRIVILEGES) 설정
--
-- 사전 작업 (셸에서):
--   createdb -U <superuser> hanacard
--
-- 사전 작업 (역할 3개 생성 — 비밀번호가 들어가므로 파일에 넣지 않음):
--   psql -d hanacard -c "CREATE ROLE agent_svc      LOGIN PASSWORD '<직접입력>';"
--   psql -d hanacard -c "CREATE ROLE app_svc        LOGIN PASSWORD '<직접입력>';"
--   psql -d hanacard -c "CREATE ROLE hanacard_admin LOGIN PASSWORD '<직접입력>';"
--
--   agent_svc      : LLM/AI 에이전트용. anon 전체 SELECT + service 실행계층 최소권한.
--   app_svc        : 담당자 대면 서비스용. service 스키마 CRUD. mart 접근 불가.
--   hanacard_admin : 스키마 소유자. 마이그레이션(psql·Alembic)·배치 전용,
--                    앱 런타임에는 사용하지 않음. 개인 OS 계정을 대체한다.
--
-- 실행: psql -v ON_ERROR_STOP=1 -d hanacard -f V001__roles_and_database.sql
-- 다음: V002 (이후는 hanacard_admin 계정으로 실행할 것 — README 참고)
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- 2. hanacard_admin에게 스키마 생성 권한
--
-- 왜 필요한가: V002~V005가 CREATE SCHEMA mart/anon 을 수행하는데, PostgreSQL에서
-- 스키마 생성은 데이터베이스에 대한 CREATE 권한을 요구한다. 기본적으로 이 권한은
-- DB 소유자에게만 있으므로, hanacard_admin으로 마이그레이션을 실행하려면 명시적
-- 부여가 필요하다.
--
-- 왜 hanacard_admin으로 실행해야 하는가: PostgreSQL은 CREATE TABLE을 "실행한
-- 계정"을 자동으로 소유자로 삼는다. 개인 계정으로 실행하면 그 사람만 이후
-- ALTER/TRUNCATE/트리거제어를 할 수 있게 되어, 팀원·배포 환경마다 소유자가
-- 달라지고 Alembic·배치가 실패한다. 소유자를 hanacard_admin 하나로 고정한다.
-- ---------------------------------------------------------------------
GRANT CREATE, CONNECT ON DATABASE :"DB_NAME" TO hanacard_admin;

-- ---------------------------------------------------------------------
-- 3. 스키마 3개 생성 (소유자 = hanacard_admin)
--
-- 왜 여기서 만드는가:
...
--   superuser가 소유자가 된다. 소유자를 hanacard_admin으로 고정해야
--   이후 마이그레이션·배치가 개인/슈퍼 계정 없이 동작한다.
-- ---------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS mart    AUTHORIZATION hanacard_admin;
CREATE SCHEMA IF NOT EXISTS anon    AUTHORIZATION hanacard_admin;
CREATE SCHEMA IF NOT EXISTS service AUTHORIZATION hanacard_admin;

-- ---------------------------------------------------------------------
-- 4. 기본 권한 (DEFAULT PRIVILEGES)
--
-- 앞으로 hanacard_admin이 만드는 service 테이블에 app_svc 권한을 자동 부여한다.
...
-- FOR ROLE hanacard_admin 을 명시한다(Alembic이 이 계정으로 접속하므로).
-- ---------------------------------------------------------------------
ALTER DEFAULT PRIVILEGES FOR ROLE hanacard_admin IN SCHEMA service
...
ALTER DEFAULT PRIVILEGES FOR ROLE hanacard_admin IN SCHEMA anon
...
--
-- SELECT has_database_privilege('hanacard_admin','hanacard','CREATE');  -- t
--
-- -- 스키마 3개가 hanacard_admin 소유로 존재하는가 (3행이어야 정상)
